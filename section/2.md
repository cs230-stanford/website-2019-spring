...
- By default, AWS instances have an EBS hard disk that is completely erased when you turn off the instance. If you would like the boot disk to persist, so that you don't need to download your code and data each time:
  - When creating a new instance, click on the Storage tab to see the boot disk.
  - Select a disk size, and unselect Delete on Termination.
- Note that you will be charged while the disk persists, so you should delete the EBS disk when you're done.
