...
Here's some more more intuition. The amount of data required depends on:

- Complexity of the task (output, number of features…)
- Complexity of the model. For instance, the amount of data needed generally increases with the number of classes in classification.
...
Try not to put too much time on data cleaning and preprocessing! Better to have time to focus on the neural networks. 
